...
Train_set <- function(i, h){
  Stock <- StockData[,i]
  slice <- createTimeSlices(Stock, initialWindow = 3 + h, horizon = 0)
  idx <- data.frame(matrix(unlist(slice[[1]][1:(196 - h)]), ncol = 3 + h, byrow = TRUE))
  idd <- matrix(nrow = 196 - h, ncol = 3 + h)
  for (i in 1 : (196 - h)){
    for (j in 1 : (3 + h)){
...
Pred_set <- function(i, h){
  Stock <- StockData[,i]
  slice <- createTimeSlices(Stock, initialWindow = 3 + h, horizon = 0)
  idy <- data.frame(matrix(unlist(slice[[1]][196:250]), ncol = 3 + h, byrow = TRUE))
  idt <- matrix(nrow = (53 - h), ncol = 3 + h)
  for (i in 1:(53 - h)){
    for (j in 1:(3 + h)){
...
for (j in 2:p)
{
  ...
  fit1 <- randomForest(X4 ~ ., data = Train_set(j,1))
  fit2 <- randomForest(X5 ~ .-X4, data = Train_set(j,2))
  fit3 <- randomForest(X6 ~ .-X5-X4, data = Train_set(j,3))
  
  step1.forecasts <- predict(fit1, newdata = Pred_set(j, 1))
  step2.forecasts <- predict(fit2, newdata = Pred_set(j, 2))
  step3.forecasts <- predict(fit3, newdata = Pred_set(j, 3))
