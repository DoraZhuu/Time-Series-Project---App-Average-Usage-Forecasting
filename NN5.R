...
Train_set <- function(i, h){
  Stock <- StockData[,i]
  slice <- createTimeSlices(Stock, initialWindow = 5 + h, horizon = 0)
  idx <- data.frame(matrix(unlist(slice[[1]][1:(194 - h)]), ncol = 5 + h, byrow = TRUE))
  idd <- matrix(nrow = 194 - h, ncol = 5 + h)
  for (i in 1 : (194 - h)){
    for (j in 1 : (5 + h)){
...
Pred_set <- function(i, h){
  Stock <- StockData[,i]
  slice <- createTimeSlices(Stock, initialWindow = 5 + h, horizon = 0)
  idy <- data.frame(matrix(unlist(slice[[1]][194:250]), ncol = 5 + h, byrow = TRUE))
  idt <- matrix(nrow = (53 - h), ncol = 5 + h)
  for (i in 1:(53 - h)){
    for (j in 1:(5 + h)){
...
for (j in 2:p)
{
  ...
  net1 <- neuralnet(X6 ~ ., data = Train_set(j,1), hidden = 3, learningrate = 0.01, stepmax = 1e+06)
  net2 <- neuralnet(X7 ~ .-X6, data = Train_set(j,2), hidden = 3, learningrate = 0.01, stepmax = 1e+06)
  net3 <- neuralnet(X8 ~ .-X6-X7, data = Train_set(j,3), hidden = 3, learningrate = 0.01, stepmax = 1e+06)
